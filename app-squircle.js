(function (globalScope) {
  ...
  function distributeAndNormalize({
    ...
    Object.entries(cornerRadiusMap)
      .sort(([, radius1], [, radius2]) => radius2 - radius1)
      .forEach(([cornerName, radius]) => {
        const corner = cornerName;
        const adjacents = ADJACENTS_BY_CORNER[corner];
  ...
  function getSvgPath({
    ...
    if (
      ...
      const normalizedRadius = Math.min(topLeftCornerRadius, roundingAndSmoothingBudget);
  ...
  function applySquircle(element) {
    ...
    const path = getSvgPath({
      width: rect.width,
      height: rect.height,
      cornerRadius: Number(element.dataset.squircleRadius || 18),
      cornerSmoothing: Number(element.dataset.squircleSmoothing || 0.85),
      preserveSmoothing: element.dataset.squirclePreserve !== 'false',
    });
  ...
  function applySquircles(selector = '[data-squircle]') {
    document.querySelectorAll(selector).forEach(applySquircle);
  }

  function bindSquircles(selector = '[data-squircle]') {
    const rerender = () => applySquircles(selector);
    ...
    globalScope.addEventListener('resize', rerender);
